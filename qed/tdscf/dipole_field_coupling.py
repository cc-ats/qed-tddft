...
def magnetic_dipole_mat(mol, scf_type='RKS'):
    ...
    if scf_type in {'GHF', 'GKS'}:
        ints = np.array([scipy.linalg.block_diag(m, m) for m in ints])
    return ints


def dipole_dot_efield(mol, grids, cavity_mode, uniform_field=True,
                      efield_file=None, scf_type=scf.RHF):
    # use charge center! important for oo and vv block
    from pyscf.tdscf.rhf import _charge_center
    with mol.with_common_orig(_charge_center(mol)):
        dip_ao = mol.intor_symmetric('int1e_r', comp=3)

    if uniform_field:
        dip_dot_efield = lib.einsum('xmn,xp->pmn', dip_ao, cavity_mode)

    else:
        coords0, efield0 = get_electric_field(efield_file)

        # test
        #cavity_size = [80, 80, 80, 1, 10]
        #ngrids, coords0, _ = creat_mesh_grids(mol, grids, grid_type=2, nxyz=cavity_size)
        #efield0 = np.zeros((ngrids, 3))
        #efield0[:, -1] = 0.01

        # get Lebedev grids
        ngrids, coords, weights = creat_mesh_grids(mol, grids, grid_type=1)
    ...
    if scf_type in {'GHF', 'GKS'}:
        dip_ao = np.array([scipy.linalg.block_diag(m, m) for m in dip_ao])
        dip_dot_efield = np.array([scipy.linalg.block_diag(m, m) for m in dip_dot_efield])
    return dip_ao, dip_dot_efield


def build_dip_ao_check(mol, ngrids, coords, weights, cav_size):
    efield = np.sqrt(2/np.prod(cav_size))
    re = coords * efield
    re = np.einsum('ix,i->ix', re, weights)
    ...
    for ip0, ip1 in lib.prange(0, ngrids, blksize):
        ao_value[ip0:ip1] = mol.eval_gto('GTOval_sph_deriv1', coords[ip0:ip1])[0]
    dip_ao = np.einsum('ip,iq,ix->xpq', ao_value, ao_value, re)

    with mol.with_common_orig((0,0,0)):
        dip_ao0 = mol.intor("int1e_r", comp=3)
    dip_ao0 *= efield

    for x in range(3):
        print_matrix('dip_ao', dip_ao[x], 10)
        print_matrix('dip_ao0', dip_ao0[x], 10)


def build_dip_ao_check2(mol, grids):
    strength = 0.01
    cavity_mode = np.ones((3, 1)) * strength

    cavity_size = [80, 80, 80, 1, 6]
    ngrids, coords0, _ = creat_mesh_grids(mol, grids, grid_type=2, nxyz=cavity_size)
    efield0 = np.zeros((ngrids, 7))
    efield0[:, 4] = strength
    efield0[:, 6] = strength
    P = np.copy(coords0) / 10
    P = np.concatenate((P, efield0), axis=1)
    np.savetxt('field.txt', P, fmt='%10.5f')
    ...
    efield = interpolate_electric_field(coords, coords0, efield0)
    #print_matrix('efield', efield, 10)
    print(np.any(np.isnan(efield)))
...
if __name__ == '__main__':
    ...
    build_dip_ao_check2(mol, mf.grids)
